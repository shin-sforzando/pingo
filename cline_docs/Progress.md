...
1. **高優先のUI/UX改善**
   - ログイン前のHeaderUIの改善（UserMenuの削除、通知アイコン→言語切り替えボタン）
...
1. **UI設計の改善点**: ログイン前のHeaderにはUserMenuと通知アイコンは不要、代わりに言語切り替えボタンが適切。
2. **UI一貫性の欠如**: `usernameInvalid` のバリデーション条件の一貫性確保が必要。
3. **パスワード検証の不備**: アカウント作成からログインに関する正常系、準正常系の統合テストがない。
4. **テストカバレッジの不足**: 認証系APIや `src/lib/firebase` 以下のソースコードに関するテストが不足。
5. **テスト用Firestoreのクリーンナップ**: テスト後にテストデータを削除し、本番環境を汚さないようにする。

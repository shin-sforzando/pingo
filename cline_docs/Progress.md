...
- Google GenAI構造化出力への移行
  - 3つのAPIエンドポイントすべてを構造化出力に移行
    ...
    - `subjects/check`: 被写体候補検証API
    - `image/check`: 画像内容チェックAPI
  - 技術的改善
    - `Type`インポートと`responseSchema`定義の追加
    - `responseMimeType: "application/json"`と`responseSchema`の設定
    - プロンプトからJSONフォーマット指定を削除（約50%短縮）
    - マークダウンコードブロック除去処理の削除
    - レスポンス処理の簡素化
  - 信頼性向上
    - JSONパースエラーのリスクを大幅に削減
    - マークダウン混入（```json）の防止
    - 確実なJSONレスポンスの保証
  - 保守性向上
    - プロンプト内のJSONフォーマット指定が不要
    - コードの重複削除と統一
    - 型安全性の向上
  - パフォーマンス向上
    - 不要な文字列処理の削除
